...
//clicakables
function Dclicker2(){$('#hideButton').click();}
function Dclicker(){$('#shimBtm').hide();}
...
$(document).keyup(function(e) {
 // if (e.keyCode === 13) $('#showButton').click();     // enter current-> search...
  //if (e.keyCode === 27) {$('.hideButton').click();  } // esc
    if (e.keyCode === 81) {$('#hideButton').click();}  // esc
...
$(showtags).html(searchtags);  tagRender(tagOpt);
}
//
//----------------------------------------------------
//----------------------------------------------------
...
function clickSearch(query){
  ...
  if(pickShow.length < 2){clickSearch2(query); return false;}else{
...
.error(function() {    $(searches).fadeIn();  
...
function getTopChars(value){  if(!pickShow){return false};
       ...
       $.each(val.content, function(key2, val2) { 
          if (val2.text != null){
            ...
            $.each(val2.elements, function(key35, val35) {
             ...
             // console.log(val35);
              paragraph.push('<p style="font-size:36;">'+val35.text+'</p>');console.log(val35.text);
            }); 
          }
        }); 
    });
...
$(topButton).fadeIn();
  $(searches).fadeOut();       $(searchButton2).fadeIn();
  $(searchButton).fadeOut();   $(shim).show();
  $(resetButton).fadeIn();    $(searches).fadeOut();
                  // $('#adBox').fade();
